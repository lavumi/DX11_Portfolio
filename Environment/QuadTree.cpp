...
void QuadTree::Initialize(UINT mapWidth, UINT mapHeight)
{
...
void QuadTree::CreateTree()
{
	...
	//문제점 : heightmap 기반으로 읽어오는 데이터는 보통 512*512
	//때문에 버텍스가 512개로 구성되고 1x1 의 노드가 511*511개로 구성되어 분할이 깔끔하지 않다.
	//우선은 맨 끝에 1줄씩 가상의 라인을 만들어 계산하자
...
void QuadTree::getGrassPos(vector<D3DXVECTOR2>& out_vertexData, Frustum * frustum)
{
	Camera::Get()->GetPosition(&cameraPos);
	scanGrassPos(out_vertexData, head, frustum);

}

void QuadTree::scanGrassPos(vector<D3DXVECTOR2>& out_vertexData, Node * node, Frustum * frustum)
{
	...
	if (node->subNodes[0] != nullptr) {
		for (UINT i = 0; i < 4; i++) {
			scanGrassPos(out_vertexData, node->subNodes[i], frustum);
		}
	}
	else
	{
		D3DXVECTOR2 position = D3DXVECTOR2((float)node->startX, (float)node->startY);
		out_vertexData.push_back(position);
	}
}
...
void QuadTree::SetNode(Node * node, UINT startX_Idx, UINT startY_Idx, UINT width, UINT height)
{
	...
	//설정된 최대 버텍스 갯수보다 포함 버텍스가 많으면 4분할 한다
		...
		for (UINT i = 0; i < 4; i++) {
			...
			SetNode(subNode
				, startX_Idx + width / 2 * (i % 2)
				, startY_Idx + height / 2 * (i / 2)
				, width / 2
				, height /2
			);
		}
...
void QuadTree::Node::getIndecies(UINT* & out_indices, UINT & out_width, UINT& out_height, UINT LOD)
{
	...
	out_width = (UINT)ceil((float)(width - 1) / (float)level) + 1;
	out_height = (UINT)ceil((float)(height - 1) / (float)level) + 1;
...
void QuadTree::SetNodeOLD(Node* node, UINT startX_Idx, UINT startY_Idx, UINT width, UINT height)
{
	node->tree = this;
